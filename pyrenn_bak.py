...
def Matrix_extend(mat):
	"""Extend existed matrice according to full Cubic 

	Args:
		mat:  original matrice
        new_mat:
            A:-->A;A^2;A^3
            B:-->B;B^2;B^3
            C:-->C;C^2;C^3
            A,B,C:-->AxB;AxC;BxC;A^2xB;A^2xC;B^2xC;C^2xA;C^2xB;AxBxC
	Returns:
		new_mat: transformed matrice
	"""
	m = int(np.shape(mat)[0])
	if len(np.shape(mat)) > 1:
		n = int(np.shape(mat)[1])

#	n1 = n  # ii, (m,n) the shape of mat; Making sure n>2
#   n2=2*n*(n-1)  #i*j
#	n2 = 3*n*(n-1)//2  # i*j
#	n3 = 0  # ii*(i+1)
#	n4 = n  # iii
#	n5 = n*(n-1)*(n-2)//6  # cn3
	n1=n*(n-1)/2	
#	new_n = n1+n2+n3+n4+n5+n
	new_n=n+n1
	new_mat = np.zeros(int(m*new_n))
	new_mat = np.reshape(new_mat, (m, new_n))  # Generate a new matrix
	print(new_n)
	n_flag = 0
	for j in range(n):
		for i in range(m):
			new_mat[i][n_flag] = mat[i][j]  # Put original mat into new mat
		n_flag = n_flag+1  # row flag
	#for j in range(n):
	#	for i in range(m):
	#		new_mat[i][n_flag]=mat[i][j]*mat[i][j]    #aij*aij
	#	n_flag=n_flag+1
    # for j in range(n-1):
    #    for i in range(m):
    #        new_mat[i][n_flag]=mat[i][j]*mat[i][j]*mat[i][j+1]  ####aij*aij*ai(j+1)
    #    n_flag=n_flag+1
	#for j in range(n):
	#	for i in range(m):
	#		new_mat[i][n_flag]=mat[i][j]*mat[i][j]*mat[i][j]    ##### aij*aij*aij
	#	n_flag=n_flag+1
...
#			if j1!=j2:
#				for i in range(m):
#					new_mat[i][n_flag]=mat[i][j1]*mat[i][j1]*mat[i][j2]
#				n_flag=n_flag+1
			if j1>j2:    
				for i in range(m):
					new_mat[i][n_flag]=mat[i][j1]/mat[i][j2]
				n_flag=n_flag+1
    # c(n,3)
#	for j1 in range(n):
...
#					for i in range(m):
#						new_mat[i][n_flag]=mat[i][j1]*mat[i][j2]*mat[i][j3]
#					n_flag=n_flag+1
	return new_mat
